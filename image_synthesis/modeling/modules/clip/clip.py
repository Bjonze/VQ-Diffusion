...
from image_synthesis.modeling.embeddings.ctx_conditioner import ScalarTokenizer
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77, 
    ...
    #all_tokens = tokenizer(texts)
    if just_token:
        return texts
    #result = torch.zeros(len(all_tokens), context_length, conte dtype=torch.long) + pad_value
    result = torch.zeros_like(texts, dtype=torch.float) + pad_value
    if with_mask:
        mask = torch.zeros(len(texts), context_length, dtype=torch.bool)
...
def tokenize_old(texts: Union[str, List[str]], context_length: int = 77, 
             add_start_and_end: bool = True, with_mask: bool = True,
             pad_value: int = 0, tokenizer=None, just_token: bool = False) -> torch.LongTensor:
    """
    Returns the tokenized representation of given input string(s)
